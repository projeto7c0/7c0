...
def insere_lista(tweets, twitter_list):
    ...
    for tweet in tweets:
        ...
        if not old_tweet:
            user = session.query(User).filter_by(list_id=twitter_list.id).filter_by(
                twitter_user_id=tweet.user.id_str).first()
            if (user):
                new_tweet = Tweet(text=tweet.full_text,
                                  ...
                                  quoted_status_id=tweet.quoted_status_id_str if hasattr(
                                      tweet, 'quoted_status_id_str') else None,
                                  retweet_count=tweet.retweet_count,
                                  reply_to_screen_name=tweet.in_reply_to_screen_name,
                                  reply_to_status_id=tweet.in_reply_to_status_id_str,
                                  in_reply_to_status_id=tweet.in_reply_to_status_id_str,
                                  retweeted_status_id=tweet.retweeted_status.id_str if hasattr(
                                      tweet, 'retweeted_status') else None,
                                  reply_count=tweet.reply_count if hasattr(
                                      tweet, 'reply_count') else None,
                                  five_min_check=True,
                                  hourly_check=True,
                                  daily_check=True,
                                  weekly_check=True,
                ...
                for hashtag in tweet.entities['hashtags']:
                    new_hashtag = Hashtag(
                        text=hashtag['text'], tweet_id=new_tweet.id)
                    session.add(new_hashtag)

                for mention in tweet.entities['user_mentions']:
                    new_mention = Mention(
                        user_str_id=mention['id_str'], screen_name=mention['screen_name'], tweet_id=new_tweet.id)
                    session.add(new_mention)

                for url in tweet.entities['urls']:
                    new_url = Url(url=url['expanded_url'],
...
def get_restricted_users():
    Session = sessionmaker(bind=engine)
    session = Session()
    return session.query(User).filter(User.active == True).filter(or_(User.protected == True, User.withheld_scope != None)).all()
...
def get_users_that_removed():
    ...
    return session.query(Tweet.user_id).filter(Tweet.erased==True).filter(Tweet.bot_tweeted==False).distinct(Tweet.user_id).all()
...
def prepara_atualizacao():
    ...
    session.query(User).update(
        {User.updated: False}, synchronize_session=False)
    session.close()
...
def insere_membros(members, twitter_list):
    ...
    for member in members:
        user = session.query(User).filter_by(
            twitter_user_id=member.id_str).filter_by(list_id=twitter_list.id).first()
        if (user == None):
            new = User(list_id=twitter_list.id,
                       ...
                       withheld_scope=member.withheld_scope if hasattr(
                           member, 'withheld_scope') else None,
                       updated=True,
...
def atualiza_membros(members):
    ...
    for member in members:
        users = session.query(User).filter_by(
            twitter_user_id=member.id_str).all()
        for user in users:
...
def update_tweets_db(updated_tweets, deleted):
    ...
    for updated_tweet in updated_tweets:
        tweet = session.query(Tweet).filter_by(
            twitter_id=updated_tweet.id_str).first()
...
def upsert_tweets(updated_tweets, deleted):
    ...
    for updated_tweet in updated_tweets:
        ...
        tweet.quoted_status_id = updated_tweet.quoted_status_id_str if hasattr(
            updated_tweet, 'quoted_status_id_str') else None
        tweet.retweet_count = updated_tweet.retweet_count
        ...
        tweet.retweeted_status_id = updated_tweet.retweeted_status.id_str if hasattr(
            updated_tweet, 'retweeted_status') else None
        tweet.reply_count = updated_tweet.reply_count if hasattr(
            updated_tweet, 'reply_count') else None
...
def get_user(user_id):
    ...
    result = session.query(User).filter(User.id==user_id[0]).first()
    session.close()
    return result
...
def update_tweet(tweet):
    ...
    tweet = session.query(Tweet).filter_by(id=tweet.id).first()
