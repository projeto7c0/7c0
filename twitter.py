...
def get_tweets(tweets_ids):
    ...
    tweets = []
    if len(ids) > 0:
        for status in api.lookup_statuses(ids, trim_user=True, include_entities=False, map=False):
            tweets.append(status)
    deleted = []
    for id in ids:
...
def get_tweets_empty(tweets_ids):
    ...
    tweets = []
    if len(ids) > 0:
        for status in api.lookup_statuses(ids, trim_user=False, include_entities=True, map=False, tweet_mode='extended'):
            tweets.append(status)
    deleted = []
    for id in ids:
        if not any(status.id_str == id for status in tweets):
